...
class SnetSDK:
    ...
    def create_dynamic_service_client(self, org_id, service_id, group_name=None, payment_channel_management_strategy=PaymentChannelManagementStrategy, options=None):
        ...
        group = service_metadata.get_group(group_name)
        strategy = payment_channel_management_strategy(self)
        grpc_output_path = PurePath(Path.resolve(
            Path(sys.argv[0]))).parent.joinpath("grpc", org_id, service_id)
        dynamic_service_client = DynamicServiceClient(
            self, service_metadata, group, org_id, service_id, grpc_output_path, strategy, options)
    ...
    def get_service_metadata(self, org_id, service_id):
        ...
        metadata_json = get_from_ipfs_and_checkhash(
            self.ipfs_client, metadata_hash)
