...
class DynamicServiceClient:
    def __init__(self, sdk, metadata, group, org_id, service_id, grpc_output_path, strategy, options):
        safe_extract_proto_from_ipfs(
            sdk.ipfs_client, metadata['model_ipfs_hash'], grpc_output_path, overwrite=True)
        ...
        for fully_qualified_method_name, (stub, request, response) in self._services.items():
            ...
            if package:
                ...
                setattr(package_object.service, service, ServiceClient(
                    sdk, metadata, group, stub, strategy, options))
                setattr(package_object.message,
                        request.DESCRIPTOR.name, request)
                setattr(package_object.message,
                        response.DESCRIPTOR.name, response)
            else:
                if not hasattr(self, "service"):
                    self.service = SimpleNamespace()
                if not hasattr(self, "message"):
                    self.message = SimpleNamespace()
                setattr(self.service, service, ServiceClient(
                    sdk, metadata, group, stub, strategy, options))
            ...
            for fully_qualified_method_name, (stub, request, response) in self._services.items():
                ...
                if method == method_name:
                    if request_type is not None or response_type is not None:
                        raise Exception(
                            "Multiple methods found for method {}. Please specify the fully qualified method name (<package>.<service>.<method>)".format(method_name))
                    else:
                        if package is None:
                            ...
                            services_object = getattr(
                                self, package, self).service
                            service_object = getattr(
                                services_object, service).service
                            method_function = getattr(service_object, method)
                            request_type = request
                            response_type = response
        # User is providing a fully qualified method name (<package>.<service>.<method>)
        else:
            if method_name in self._services:
                ...
                raise Exception(
                    "No method found for fully qualified name {}".format(method_name))

        if request_type is None or response_type is None:
            raise Exception(
                "No method {} found for given service".format(method_name))
