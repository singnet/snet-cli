...
class MPEContract:
    ...
    def get_past_open_channels(self, account, service, starting_block_number=0, to_block_number=None):
        ...
        channels_opened = list(filter(lambda channel: channel.sender == account.address and channel.signer == account.signer_address and channel.recipient ==
                                      service.group["payment_address"], [web3.utils.events.get_event_data(event_abi, l)["args"] for l in logs]))
