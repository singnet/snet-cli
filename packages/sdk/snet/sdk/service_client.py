...
class ServiceClient:
    def __init__(self, sdk, metadata, group, service_stub, payment_channel_management_strategy, options):
        ...
        self.grpc_channel = grpc.intercept_channel(
            self._base_grpc_channel, generic_client_interceptor.create(self._intercept_call))
    ...
    def _generate_grpc_stub(self, service_stub):
        ...
        disable_blockchain_operations = self.options.get(
    ...
    def _get_grpc_channel(self):
        ...
        if endpoint is None:
            endpoint = self.metadata.get_endpoints_for_group(
                self.group["group_name"])[0]
        ...
        if endpoint_object.scheme == "http":
            ...
            raise ValueError('Unsupported scheme in service metadata ("{}")'.format(
    ...
    def _get_service_call_metadata(self):
        ...
        amount = channel.state["last_signed_amount"] + \
            int(self.metadata["pricing"]["price_in_cogs"])
        message = web3.Web3.soliditySha3(
            ["address", "uint256", "uint256", "uint256"],
            [self.sdk.mpe_contract.contract.address,
                channel.channel_id, channel.state["nonce"], amount]
        )
        signature = bytes(self.sdk.web3.eth.account.signHash(
            defunct_hash_message(message), self.sdk.account.signer_private_key).signature)
        metadata = [
    ...
    def load_open_channels(self):
        ...
        new_payment_channels = self.sdk.mpe_contract.get_past_open_channels(
            self.sdk.account, self, self.last_read_block)
    ...
    def open_channel(self, amount, expiration):
        receipt = self.sdk.mpe_contract.open_channel(
            self.sdk.account, self, amount, expiration)
        return self._get_newly_opened_channel(receipt)

    def deposit_and_open_channel(self, amount, expiration):
        receipt = self.sdk.mpe_contract.deposit_and_open_channel(
            self.sdk.account, self, amount, expiration)
        return self._get_newly_opened_channel(receipt)

    def _get_newly_opened_channel(self, receipt):
        open_channels = self.sdk.mpe_contract.get_past_open_channels(
            self.sdk.account, self, receipt["blockNumber"], receipt["blockNumber"])
