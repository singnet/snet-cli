...
class ServiceClient:
    def __init__(
        ...
        group,
        service_stub,
        payment_channel_management_strategy,
        options,
    ):
        self.sdk = sdk
    ...
    def _get_grpc_channel(self):
        ...
        if endpoint_object.port is not None:
            channel_endpoint = (
                endpoint_object.hostname + ":" + str(endpoint_object.port)
            )
        else:
    ...
    def _get_service_call_metadata(self):
        ...
        amount = channel.state["last_signed_amount"] + int(
            self.group["pricing"][0]["price_in_cogs"]
        )
        ...
        signature = bytes(
            self.sdk.web3.eth.account.signHash(
                defunct_hash_message(message), self.sdk.account.signer_private_key
            ).signature
        )
        metadata = [
