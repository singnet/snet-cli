...
class MPEChannelCommand(OrganizationCommand):
    def _get_persistent_mpe_dir(self):
        ...
        return Path.home().joinpath(
            ".snet", "mpe_client", "%s_%s" % (mpe_address, registry_address)
        )
    ...
    def _get_service_info_file(self, org_id, service_id):
        return os.path.join(
            self._get_service_base_dir(org_id, service_id), "service_info.pickle"
        )
    ...
    def is_service_initialized(self):
        return os.path.isfile(
            self._get_service_info_file(self.args.org_id, self.args.service_id)
        )
    ...
    def _init_or_update_org_if_needed(self, metadata, org_registration):
        ...
        self._printerr("# Initialiaze service with org_id=%s" % self.args.org_id)
    ...
    def _check_channel_is_mine(self, channel):
        if (
            channel["sender"].lower() != self.ident.address.lower()
    ...
    def _open_channel_for_org(self, metadata):
        ...
        if mpe_cogs < self.args.amount:
            raise Exception(
                "insufficient funds. "
                "Your MPE balance is %s AGI " % cogs2stragi(mpe_cogs)
            )
    ...
    def _channel_extend_add_funds_with_channel_id(self, channel_id):
        ...
        if self.args.expiration is None:
            self.transact_contract_command(
                "MultiPartyEscrow", "channelAddFunds", [channel_id, self.args.amount]
            )
            return
        ...
        if self.args.amount is None:
            self.transact_contract_command(
                "MultiPartyEscrow", "channelExtend", [channel_id, expiration]
            )
            return
    ...
    def _smart_get_initialized_channel_for_org(
        self, metadata, filter_by, is_try_initailize=True
    ):
        """
