...
class MPEChannelCommand(OrganizationCommand):
    ...
    def get_service_spec_dir(self, org_id, service_id):
        ...
        return self._get_service_base_dir(org_id,
                                          service_id).joinpath("service")
    ...
    def is_service_initialized(self):
        return os.path.isfile(
            self._get_service_info_file(self.args.org_id,
    ...
    def _get_channels_info_file(self, org_id):
        return os.path.join(self._get_org_base_dir(org_id),
    ...
    def _check_mpe_address_metadata(self, metadata):
        ...
        if str(mpe_address).lower() != str(metadata["mpe_address"]).lower():
            raise Exception(
                "MultiPartyEscrow contract address from metadata %s do not correspond to current MultiPartyEscrow address %s"
    ...
    def _init_or_update_org_if_needed(self, metadata, org_registration):
        ...
        self._printerr("# Initialiaze service with org_id=%s" %
                       self.args.org_id)
        ...
        try:
            ...
            metadata.save_pretty(
                os.path.join(org_dir, "organization_metadata.json"))
        except:
    ...
    def _init_or_update_registered_org_if_needed(self):
        ...
        if self.is_org_initialized():
            ...
            # metadataURI will be in old_reg only for service which was initialized from registry (not from metadata)
            # we do nothing for services which were initialized from metadata
    ...
    def _check_channel_is_mine(self, channel):
        if (channel["sender"].lower() != self.ident.address.lower()
                and channel["signer"].lower() != self.ident.address.lower()):
            raise Exception(
                "Channel does not correspond to the current Ethereum identity "
                + "(address=%s sender=%s signer=%s)" % (
    ...
    def _init_channel_from_metadata(self, metadata, org_registration):
        ...
        if group_id is None:
            raise Exception(
                "Channel %i does not correspond to the given metadata.\n" %
                channel_id +
                "We can't find the following group_id in metadata: " +
    ...
    @staticmethod
    def _expiration_str_to_blocks(expiration_str, current_block):
        s = expiration_str
    ...
    def _get_expiration_from_args(self):
        ...
        if rez > current_block + 1036800 and not self.args.force:
            ...
            raise Exception(
                "You try to set expiration time too far in the future: approximately %i days. "
                % d + "Set --force parameter if your really want to do it.")
        return rez
    ...
    def _open_channel_for_org(self, metadata):
        ...
        params = [
            channel_info["signer"],
            channel_info["recipient"],
            channel_info["groupId"],
            self.args.amount,
            expiration,
        ]
        ...
        if len(rez[1]) != 1 or rez[1][0]["event"] != "ChannelOpen":
            raise Exception(
                "We've expected only one ChannelOpen event after openChannel. "
                "Make sure that you use correct MultiPartyEscrow address")
    ...
    def _initialize_already_opened_channel(self, metadata, sender, signer):
        ...
        for i in sorted(channels_ids):
            ...
            if channel["signer"].lower() == signer.lower():
                self._printerr(
                    "# Channel with given sender, signer and group_id is already exists we simply initialize it (channel_id = %i)"
                    % channel["channelId"])
    ...
    def channel_claim_timeout_all(self):
        ...
        for channel_id in channels_ids:
            ...
            if rez["value"] > 0 and rez[
                    "expiration"] < self.ident.w3.eth.blockNumber:
                self.transact_contract_command("MultiPartyEscrow",
                                               "channelClaimTimeout",
                                               [channel_id])
    ...
    def check_new_expiration_from_blockchain(self, channel_id, new_expiration):
        ...
        if new_expiration < channel["expiration"]:
            raise Exception(
                "New expiration (%i) is smaller then old one (%i)" %
                (new_expiration, channel["expiration"]))

    def _smart_get_initialized_channel_for_org(self,
                                               metadata,
                                               filter_by,
        ...
        channels = [
            c for c in channels
            if c[filter_by].lower() == self.ident.address.lower()
        ...
        if len(channels) == 0 and is_try_initailize:
            ...
            self._initialize_already_opened_channel(metadata,
                                                    self.ident.address,
                                                    self.ident.address)
            return self._smart_get_initialized_channel_for_org(
                metadata, filter_by, is_try_initailize=False)

        if len(channels) == 0:
            raise Exception(
                "Cannot find initialized channel for service with org_id=%s service_id=%s and signer=%s"
        ...
        if self.args.channel_id is None:
            if len(channels) > 1:
                ...
                raise Exception(
                    "We have several initialized channel: %s. You should use --channel-id to select one"
                    % str(channel_ids))
            return channels[0]
        for channel in channels:
            if channel["channelId"] == self.args.channel_id:
                return channel
        raise Exception(
            "Channel %i has not been initialized or your are not the sender/signer of it"
            % self.args.channel_id)
    ...
    def _get_service_registration(self):
        ...
        rez = self.call_contract_command("Registry",
                                         "getServiceRegistrationById", params)
        if not rez[0]:
            raise Exception(
                "Cannot find Service with id=%s in Organization with id=%s" %
    ...
    def _init_or_update_service_if_needed(self, metadata,
        ...
        if self.is_service_initialized():
            if self.is_service_metadataURI_has_changed(service_registration):
                ...
                return  # we do nothing
        self._printerr(
            "# Initialize service with org_id=%s and service_id=%s" %
            (self.args.org_id, self.args.service_id))
        self._check_mpe_address_metadata(metadata)
        service_dir = self.get_service_spec_dir(self.args.org_id,
                                                self.args.service_id)
    ...
    def _init_or_update_registered_service_if_needed(self):
        ...
        if self.is_service_initialized():
            ...
            # if metadataURI hasn't been changed we do nothing
            if not self.is_service_metadataURI_has_changed(
                    service_registration):
                return
        else:
            service_registration = self._get_service_registration()
