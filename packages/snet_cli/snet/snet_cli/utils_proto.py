...
def import_all_grpc_methods_from_dir(proto_dir):
    ...
    grpc_pyfiles = [str(os.path.basename(p))
                    for p in proto_dir.glob("*_pb2_grpc.py")]

    found_methods = dict()
    # <SERVICE>_pb2_grpc.py import <SERVICE>_pb2.py so we are forced to add proto_dir to path
    with add_to_path(str(proto_dir)):
        for grpc_pyfile in grpc_pyfiles:
            is_found, imported_methods = _import_protobuf_from_file(
                grpc_pyfile)
            if is_found:
...
def import_protobuf_from_dir(proto_dir, method_name, service_name=None):
    ...
    if (len(good_rez) == 0):
        raise Exception(
            "Error while loading protobuf. Cannot find method=%s" % method_name)
    if (len(good_rez) > 1):
        if (service_name):
            raise Exception("Error while loading protobuf. Found method %s.%s in multiply .proto files. We don't support packages yet!" % (
                service_name, method_name))
        else:
            raise Exception(
                "Error while loading protobuf. Found method %s in multiply .proto files. You could try to specify service_name." % method_name)
    return good_rez[0]
...
def _import_protobuf_from_file(grpc_pyfile):
    ...
    all_service_names = [stub_name[:-4]
                         for stub_name in dir(pb2_grpc) if stub_name.endswith("Stub")]

    found_services = dict()
    for service_name in all_service_names:
        service_descriptor = getattr(
        ...
        for method in service_descriptor.methods:
            ...
            found_services[method.full_name] = (
