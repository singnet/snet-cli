...
def import_protobuf_from_dir(proto_dir, method_name, service_name=None):
    ...
    grpc_pyfiles = [
        str(os.path.basename(p)) for p in proto_dir.glob("*_pb2_grpc.py")
    ]
    ...
    for grpc_pyfile in grpc_pyfiles:
        ...
        if is_found:
            good_rez.append(rez)
    if len(good_rez) == 0:
        raise Exception("Error while loading protobuf. Cannot find method=%s" %
                        method_name)
    if len(good_rez) > 1:
        if service_name:
            raise Exception(
                "Error while loading protobuf. Found method %s.%s in multiply .proto files. "
...
def _import_protobuf_from_file(grpc_pyfile, method_name, service_name=None):
    ...
    all_service_names = [
        stub_name[:-4] for stub_name in dir(pb2_grpc)
        if stub_name.endswith("Stub")
    ]
    ...
    if len(found_services) > 1:
        raise Exception(
            "Error while loading protobuf. We found methods %s in multiply services [%s]."
...
def switch_to_json_payload_encoding(call_fn, response_class):
    ...
    def json_serializer(*args, **kwargs):
        return bytes(
            json_format.MessageToJson(args[0],
                                      True,
