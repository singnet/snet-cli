...
device.""")
    ...
    parser.add_argument(
        "--issuerKey", help="Issuer key (hex encoded, default is batch 1)")
...
def getDeployedSecretV2(dongle, masterPrivate, targetId, issuerKey):
    ...
    # identify
    apdu = bytearray([0xe0, 0x04, 0x00, 0x00]) + \
        bytearray([len(targetid)]) + targetid
    dongle.exchange(apdu)

    # walk the chain
    nonce = os.urandom(8)
    apdu = bytearray([0xe0, 0x50, 0x00, 0x00]) + \
        bytearray([len(nonce)]) + nonce
    ...
    certificate = bytearray([len(testMasterPublic)]) + \
        testMasterPublic + bytearray([len(signature)]) + signature
    apdu = bytearray([0xE0, 0x51, 0x00, 0x00]) + \
        bytearray([len(certificate)]) + certificate
    dongle.exchange(apdu)

    # provide the ephemeral certificate
    ephemeralPrivate = PrivateKey()
    ephemeralPublic = bytearray(
        ephemeralPrivate.pubkey.serialize(compressed=False))
    dataToSign = bytes(bytearray([0x11]) + nonce +
                       deviceNonce + ephemeralPublic)
    signature = testMaster.ecdsa_sign(bytes(dataToSign))
    signature = testMaster.ecdsa_serialize(signature)
    certificate = bytearray([len(ephemeralPublic)]) + \
        ephemeralPublic + bytearray([len(signature)]) + signature
    apdu = bytearray([0xE0, 0x51, 0x80, 0x00]) + \
        bytearray([len(certificate)]) + certificate
    dongle.exchange(apdu)
    ...
    while True:
        if index == 0:
            certificate = bytearray(dongle.exchange(
                bytearray.fromhex('E052000000')))
        elif index == 1:
            certificate = bytearray(dongle.exchange(
                bytearray.fromhex('E052800000')))
        else:
            break
        offset = 1
        certificateHeader = certificate[offset: offset + certificate[offset-1]]
        offset += certificate[offset-1] + 1
        certificatePublicKey = certificate[offset: offset +
                                           certificate[offset-1]]
        offset += certificate[offset-1] + 1
        certificateSignatureArray = certificate[offset: offset +
                                                certificate[offset-1]]
        ...
        if index == 0:
            ...
            certificateSignedData = bytearray(
                [0x02]) + certificateHeader + certificatePublicKey
            # Could check if the device certificate is signed by the issuer public key
        # ephemeral key certificate
        else:
            certificateSignedData = bytearray(
                [0x12]) + deviceNonce + nonce + certificatePublicKey
    ...
    secret = last_pub_key.ecdh(
        binascii.unhexlify(ephemeralPrivate.serialize()))
    if targetId & 0xF == 0x2:
        return secret[0:16]
...
if __name__ == '__main__':
    ...
    secret = getDeployedSecretV2(dongle, bytearray.fromhex(
        args.rootPrivateKey), args.targetId, args.issuerKey)
    if secret != None:
        try:
    ...
    if genuine:
        ...
        if 'mcuHash' in version:
            print("MCU Hash " +
                  binascii.hexlify(version['mcuHash']).decode('ascii'))
    else:
