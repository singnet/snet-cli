...
class HexLoader:
    ...
    def __init__(self, card, cla=0xF0, secure=False, mutauth_result=None, relative=True, cleardata_block_len=None):
        ...
        if type(mutauth_result) is dict and 'ecdh_secret' in mutauth_result:
            self.scp_enc_key = self.scp_derive_key(
                mutauth_result['ecdh_secret'], 0)[0:16]
            self.scp_enc_iv = b"\x00" * 16
            self.scp_mac_key = self.scp_derive_key(
                mutauth_result['ecdh_secret'], 1)[0:16]
            self.scp_mac_iv = b"\x00" * 16
            self.scpVersion = 3
            self.max_mtu = 0xFE
            if not self.card is None:
                self.max_mtu = min(
                    self.max_mtu, self.card.apduMaxDataSize() & 0xF0)
    ...
    def crcSegment(self, offsetSegment, lengthSegment, crcExpected):
        data = b'\x08' + struct.pack('>H', offsetSegment) + struct.pack(
            '>I', lengthSegment) + struct.pack('>H', crcExpected)
        self.exchange(self.cla, 0x00, 0x00, 0x00, data)
    ...
    def createAppNoInstallParams(self, appflags, applength, appname, icon=None, path=None, iconOffset=None, iconSize=None, appversion=None):
        data = b'\x0B' + struct.pack('>I', applength) + struct.pack(
            '>I', appflags) + struct.pack('>B', len(appname)) + appname
    ...
    def createApp(self, code_length, data_length=0, install_params_length=0, flags=0, bootOffset=1):
        ...
        self.createappParams = struct.pack(
            '>IIIII', code_length, data_length, install_params_length, flags, bootOffset)
    ...
    def getVersion(self):
        ...
        result = {}
        offset = 0
        result['targetId'] = (response[offset] << 24) | (
            response[offset + 1] << 16) | (response[offset + 2] << 8) | response[offset + 3]
        offset += 4
        result['osVersion'] = response[offset + 1: offset +
                                       1 + response[offset]].decode('utf-8')
        offset += 1 + response[offset]
        offset += 1
        result['flags'] = (response[offset] << 24) | (
            response[offset + 1] << 16) | (response[offset + 2] << 8) | response[offset + 3]
        offset += 4
        result['mcuVersion'] = response[offset + 1: offset +
                                        1 + response[offset] - 1].decode('utf-8')
        offset += 1 + response[offset]
        ...
        if len(response) > 0:
            if response[0] != 0x01:
                ...
                while offset != len(response):
                    item = {}
                    offset += 1
                    item['name'] = response[offset + 1: offset +
                                            1 + response[offset]].decode('utf-8')
                    offset += 1 + response[offset]
                    item['flags'] = (response[offset] << 24) | (
                        response[offset + 1] << 16) | (response[offset + 2] << 8) | response[offset + 3]
                    offset += 4
    ...
    def getMemInfo(self):
        ...
        item = {}
        offset = 0
        item['systemSize'] = (response[offset] << 24) | (
            response[offset + 1] << 16) | (response[offset + 2] << 8) | response[offset + 3]
        offset += 4
        item['applicationsSize'] = (response[offset] << 24) | (
            response[offset + 1] << 16) | (response[offset + 2] << 8) | response[offset + 3]
        offset += 4
        item['freeSize'] = (response[offset] << 24) | (
            response[offset + 1] << 16) | (response[offset + 2] << 8) | response[offset + 3]
        offset += 4
        item['usedAppSlots'] = (response[offset] << 24) | (
            response[offset + 1] << 16) | (response[offset + 2] << 8) | response[offset + 3]
        offset += 4
        item['totalAppSlots'] = (response[offset] << 24) | (
            response[offset + 1] << 16) | (response[offset + 2] << 8) | response[offset + 3]
        return item
        ...
        for area in areas:
            ...
            while (length > 0):
                if length > max_length_per_apdu - LOAD_SEGMENT_CHUNK_HEADER_LENGTH - MIN_PADDING_LENGTH - SCP_MAC_LENGTH:
                    chunkLen = max_length_per_apdu - LOAD_SEGMENT_CHUNK_HEADER_LENGTH - \
                        MIN_PADDING_LENGTH - SCP_MAC_LENGTH
                    if (chunkLen % 16) != 0:
    ...
    def setupCustomCA(self, name, public):
        data = b'\x12' + struct.pack('>B', len(name)) + \
            name.encode() + struct.pack('>B', len(public)) + public
        self.exchange(self.cla, 0x00, 0x00, 0x00, data)
