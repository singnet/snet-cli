        ...
        for data in file:
            ...
            if recordType == 0x00:
                ...
                if startFirst == None:
                    ...
                    current = startFirst
                if address != current:
                    self._addArea(IntelHexArea(
                        (startZone << 16) + startFirst, zoneData))
                    zoneData = ""
                    startFirst = address
                    current = address
            ...
            if recordType == 0x05:
                self.bootAddr = ((data[4] & 0xFF) << 24) + ((data[5] & 0xFF)
...
class IntelHexPrinter:
    def addArea(self, startaddress, data, insertFirst=False):
        ...
        if (insertFirst):
            ...
            # order by start address
            self.areas = insertAreaSorted(
                self.areas, IntelHexArea(startaddress, data))
    ...
    def _emit_binary(self, file, bin):
        ...
        s = (":" + binascii.hexlify(bin) +
             hex(0x100+cks)[3:] + self.eol).upper()
        if (file != None):
            file.write(s)
        else:
            print(s)
            ...
            while off < len(area.data):
                ...
                if ((off & 0xFFFF0000) != (oldoff & 0xFFFF0000)):
                    self._emit_binary(file, bytearray(
                        ("02000004" + hex(0x10000+(area.start >> 16))[3:7]).decode('hex')))

                # emit data record
                if (off+blocksize > len(area.data)):
                    self._emit_binary(file, bytearray((hex(0x100+(len(area.data)-off))[3:] + hex(0x10000+off+(
                        area.start & 0xFFFF))[3:] + "00").decode('hex')) + area.data[off:len(area.data)])
                else:
                    self._emit_binary(file, bytearray((hex(0x100+blocksize)[3:] + hex(0x10000+off+(
                        area.start & 0xFFFF))[3:] + "00").decode('hex')) + area.data[off:off+blocksize])

                oldoff = off
                off += blocksize

        bootAddrHex = hex(0x100000000+self.bootAddr)[3:]
        s = ":04000005"+bootAddrHex + \
            hex(0x100+self.checksum(bytearray(("04000005" +
                                               bootAddrHex).decode('hex'))))[3:]+self.eol
        if (file != None):
            file.write(s)
        else:
            print(s)
