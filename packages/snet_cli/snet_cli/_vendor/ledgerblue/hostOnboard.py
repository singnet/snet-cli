...
""")
    ...
    parser.add_argument(
        "--pin", help="Set a PINs to backup the seed for future use")
...
if __name__ == '__main__':
    ...
    args.prefix = enter_if_none_and_normalize(
        "Derivation prefix: ", args.prefix)
    ...
    if args.prefix:
        apdudata += bytearray([len(args.prefix)]) + \
            bytearray(args.prefix, 'utf8')
    else:
        apdudata += bytearray([0])

    if args.passphrase:
        apdudata += bytearray([len(args.passphrase)]) + \
            bytearray(args.passphrase, 'utf8')
    else:
        apdudata += bytearray([0])

    if args.words:
        apdudata += bytearray([len(args.words)]) + \
            bytearray(args.words, 'utf8')
    else:
        apdudata += bytearray([0])
