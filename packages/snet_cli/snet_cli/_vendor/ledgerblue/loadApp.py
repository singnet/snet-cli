...
def get_argparser():
    ...
    parser.add_argument(
        "--targetId", help="The device's target ID (default is Ledger Blue)", type=auto_int)
    parser.add_argument(
        "--fileName", help="The application hex file to be loaded onto the device")
...
repeated""", action='append')
    parser.add_argument(
        "--appName", help="The name to give the application after loading it")
    parser.add_argument(
        "--signature", help="A signature of the application (hex encoded)")
    parser.add_argument(
        "--signApp", help="Sign application with provided signPrivateKey", action='store_true')
    parser.add_argument(
        "--appFlags", help="The application flags", type=auto_int)
    parser.add_argument(
...
a random one will be generated)""")
    parser.add_argument("--signPrivateKey",
                        help="Set the private key used to sign the loaded app")
    parser.add_argument("--apdu", help="Display APDU log", action='store_true')
    parser.add_argument(
        "--deployLegacy", help="Use legacy deployment API", action='store_true')
    parser.add_argument(
        "--apilevel", help="Use given API level when interacting with the device", type=auto_int)
    parser.add_argument(
        "--delete", help="Delete the app with the same name before loading the provided one", action='store_true')
    parser.add_argument(
        "--params", help="Store icon and install parameters in a parameter section before the code", action='store_true')
    parser.add_argument(
        "--tlv", help="Use install parameters for all variable length parameters", action='store_true')
    parser.add_argument(
        "--dataSize", help="The code section's size in the provided hex file (to separate data from code, if not provided the whole allocated NVRAM section for the application will remain readonly.", type=auto_int)
    parser.add_argument(
        "--appVersion", help="The application version (as a string)")
    parser.add_argument(
        "--offline", help="Request to only output application load APDUs", action="store_true")
    parser.add_argument("--installparamsSize",
                        help="The loaded install parameters section size (when parameters are already included within the .hex file.", type=auto_int)
    parser.add_argument(
        "--tlvraw", help="Add a custom install param with the hextag:hexvalue encoding", action='append')
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    if args.apilevel >= 5:
        ...
        if args.path != None:
            if len(args.path) > 1:
                print(
                    "Multiple path levels not supported using this API level, ignoring")
            else:
    ...
    if not args.offline:
        ...
        if args.deployLegacy:
            secret = getDeployedSecretV1(dongle, bytearray.fromhex(
                args.rootPrivateKey), args.targetId)
        else:
            secret = getDeployedSecretV2(dongle, bytearray.fromhex(
                args.rootPrivateKey), args.targetId)

    loader = HexLoader(dongle, 0xe0, not(args.offline), secret,
                       cleardata_block_len=cleardata_block_len)
    ...
    if (args.tlv):
        ...
        if (args.tlvraw):
            for tlvraw in args.tlvraw:
                ...
                installparams += encodetlv(int(hextag, 16),
                                           binascii.unhexlify(hexvalue))
        ...
        if (not (args.appFlags & 2)) and (args.installparamsSize is None or args.installparamsSize == 0):
            ...
            if not args.appVersion is None:
                installparams += encodetlv(BOLOS_TAG_APPVERSION,
            ...
            param_start = printer.maxAddr()+(PAGE_ALIGNMENT-(args.dataSize %
        ...
        loader.createApp(code_length, args.dataSize, paramsSize,
                         args.appFlags, args.bootAddr | 1)
    elif (args.params):
        ...
        param_start = printer.minAddr()-len(paramsSectionContent)-(DEFAULT_ALIGNMENT -
        ...
        loader.createAppNoInstallParams(args.appFlags, appLength, args.appName, None, path, 0, len(
            paramsSectionContent), args.appVersion)
    else:
        # account for added regions (install parameters, icon ...)
        appLength = printer.maxAddr() - printer.minAddr()
        loader.createAppNoInstallParams(
            args.appFlags, appLength, args.appName, args.icon, path, None, None, args.appVersion)
    ...
    if (signature == None and args.signApp):
        masterPrivate = PrivateKey(
            bytes(bytearray.fromhex(args.signPrivateKey)))
