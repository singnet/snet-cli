...
Issuer keypair used by Ledger to sign the device's Issuer Certificate""", default="perso_11")
    parser.add_argument(
        "--firmware", help="A reference to the firmware to load")
    parser.add_argument(
        "--targetId", help="The device's target ID (default is Ledger Blue)", type=auto_int)
    parser.add_argument(
        "--firmwareKey", help="A reference to the firmware key to use")
    return parser
...
def serverQuery(request, url):
    ...
    req = urllib2.Request(
        args.url, data, {"Content-type": "application/octet-stream"})
...
if __name__ == '__main__':
    ...
    remotePublicKeySignature = response.response[offset: offset +
                                                 remotePublicKeySignatureLength]

    certificate = bytearray([len(remotePublicKey)]) + remotePublicKey + \
        bytearray([len(remotePublicKeySignature)]) + remotePublicKeySignature
