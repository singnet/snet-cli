...
def add_p_org_id(p):
    p.add_argument("org_id", help="Id of the Organization", metavar="ORG_ID")
    ...
    if len(fns) > 0:
        ...
        for fn in fns:
            ...
            for i in fn["named_inputs"]:
                fn_p.add_argument(
                    "contract_named_input_{}".format(i[0]),
...
def add_p_endpoint(p):
    p.add_argument("endpoint", help="Service endpoint", metavar="ENDPOINT")
...
def add_p_expiration(p, is_optional):
    ...
    if is_optional:
        ...
        p.add_argument("expiration", help=h, metavar="EXPIRATION")
    p.add_argument(
        "--force",
        action="store_true",
...
def add_mpe_service_options(parser):
    ...
    p.add_argument("group_name", help="group name for fixed price method")
    p.add_argument(
        "price",
        type=stragi2cogs,
        help="Set fixed price in AGI token for all methods",
    ...
    p.add_argument("service_name", help="service name")
    p.add_argument("method", help="method for which price need to be set")
