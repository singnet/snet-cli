...
class MPEAccountCommand(BlockchainCommand):
    ...
    def print_agi_and_mpe_balances(self):
        ...
        agi_cogs = self.call_contract_command(
            "SingularityNetToken", "balanceOf", [account])
        mpe_cogs = self.call_contract_command(
            "MultiPartyEscrow",    "balances",  [account])
    ...
    def deposit_to_mpe(self):
        ...
        already_approved = self.call_contract_command(
            "SingularityNetToken", "allowance", [self.ident.address, mpe_address])
        if (already_approved < amount):
            self.transact_contract_command(
                "SingularityNetToken", "approve", [mpe_address, amount])
        self.transact_contract_command("MultiPartyEscrow", "deposit", [amount])

    def withdraw_from_mpe(self):
        self.transact_contract_command(
            "MultiPartyEscrow", "withdraw", [self.args.amount])
