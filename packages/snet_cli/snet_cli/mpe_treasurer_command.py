...
class MPETreasurerCommand(MPEClientCommand):
    ...
    def _blockchain_claim(self, payments):
        for payment in payments:
            ...
            if len(sig) != 65:
                raise Exception(
                    "Length of signature is incorrect: %i instead of 65" %
                    (len(sig)))
            v, r, s = int(sig[-1]), sig[:32], sig[32:64]
            v = v % 27 + 27
            params = [channel_id, amount, amount, v, r, s, False]
            self.transact_contract_command("MultiPartyEscrow", "channelClaim",
                                           params)
    ...
    def _start_claim_channels(self, grpc_channel, channels_ids):
        ...
        for channel_id in channels_ids:
            if (channel_id not in unclaimed_payments_dict
                    or unclaimed_payments_dict[channel_id]["amount"] == 0):
                self._printout(
                    "There is nothing to claim for channel %i, we skip it" %
                    channel_id)
                continue
            blockchain = self._get_channel_state_from_blockchain(channel_id)
            if unclaimed_payments_dict[channel_id]["nonce"] != blockchain[
                    "nonce"]:
                self._printout(
                    "Old payment for channel %i is still in progress. Please run claim for this channel later."
                    % channel_id)
                continue
    ...
    def _claim_in_progress_and_claim_channels(self, grpc_channel, channels):
        ...
        if len(payments):
            self._printout(
                "There are %i payments in 'progress' (they haven't been claimed in blockchain). We will claim them."
                % len(payments))
    ...
    def claim_almost_expired_channels(self):
        ...
        for p in unclaimed_payments:
            ...
            blockchain = self._get_channel_state_from_blockchain(channel_id)
            if (blockchain["expiration"] < self.ident.w3.eth.blockNumber +
