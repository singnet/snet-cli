...
class SDKCommand(MPEServiceCommand):
    def generate_client_library(self):
        ...
        # Compile proto files
            compile_proto(Path(proto_temp_dir_path),
                          library_dir_path, target_language=self.args.language)

        self._printout('client libraries for service with id "{}" in org with id "{}" generated at {}'.format(
