...
def get_contract_address(cmd, contract_name, error_message=None):
    ...
    error_message = (
        error_message or
        'Fail to read %s address from "networks", you should specify address by yourself via --%s_at parameter'
...
def get_field_from_args_or_session(config, args, field_name):
    ...
    if rez:
        return rez
    raise Exception(
        "Fail to get default_%s from config, should specify %s via --%s parameter"
