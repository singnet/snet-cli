...
def add_mpe_channel_options(parser):
    ...
    p = subparsers.add_parser("add-for-service", help="Add funds for the channel for the given service")
    p.set_defaults(fn="channel_add_funds_for_service")
    add_p_service_in_registry(p)
    p.add_argument("amount", type=stragi2cogs, help="Amount of AGI tokens to add to the channel")
    add_p_mpe_address_opt(p)
    add_transaction_arguments(p)
    add_p_group_name(p)
    add_p_channel_id_opt(p)
    ...
    p = subparsers.add_parser("extend-for-service", help="Set new expiration for the channel for the given service")
    p.set_defaults(fn="channel_extend_for_service")
    add_p_service_in_registry(p)
    add_p_expiration(p, is_optional = False)
    add_p_mpe_address_opt(p)
    add_transaction_arguments(p)
