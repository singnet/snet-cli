...
class CustomParser(argparse.ArgumentParser):
    ...
    def _parse_known_args(self, arg_strings, *args, **kwargs):
        if self.default_choice and not len(list(
...
def get_root_parser(config):
    ...
    parser.add_argument("--print-traceback",
                        action='store_true',
                        help="Do not catch last exception and "
...
def add_root_options(parser, config):
    ...
    p = subparsers.add_parser("channel",
                              help="Interact with SingularityNET "
                                   "payment channels")
    ...
    contract_p = subparsers.add_parser("contract",
                                       help="Interact with contracts at "
                                            "a low level")
...
def add_identity_options(parser, config):
    ...
    p.add_argument("--keystore-path",
                   help="Path of the JSON encrypted file for "
                        "'keystore' identity_type")
    p.add_argument("--network",
                   help="Network this identity will be bind to "
                        "(obligatory for 'rpc' identity_type, "
                        "optional for others)")
    ...
    for identity_name in identity_names:
        p = subparsers.add_parser(identity_name,
                                  help="Switch to {} identity".format(
                                      identity_name))
...
def add_network_options(parser, config):
    ...
    p.add_argument("--default-gas-price",
                   default="medium",
                   help="Default gas price (in wei) or gas price strategy "
                        "('fast' ~1min, "
                        "'medium' ~5min "
                        "or 'slow' ~60min), "
                        "default is 'medium'")
    ...
    for network_name in network_names:
        p = subparsers.add_parser(network_name,
                                  help="Switch to {} network".format(
                                      network_name))
...
def add_contract_options(parser):
    ...
    for path in Path(__file__).absolute().parent.joinpath("resources",
                                                          "contracts",
                                                          "abi").glob("*json"):
        contract_name = re.search(r"([^.]*)\.json",
                                  os.path.basename(path)).group(1)
        contract_p = subparsers.add_parser(contract_name,
...
def add_organization_options(parser):
    ...
    p = subparsers.add_parser("list-my",
                              help="Print organization which has the current "
                                   "identity as the owner or as a member")
    ...
    pm.add_argument("--auto",
                    action='store_true',
                    help="Generate organization Id "
                         "(by default random id is generated)")
...
def add_contract_function_options(parser, contract_name):
    ...
    for fn in filter(lambda e: e["type"] == "function", contract_def["abi"]):
        fns.append({
            "name": fn["name"],
            "named_inputs": [(i["name"], i["type"]) for i in fn["inputs"] if
                             i["name"] != ""],
            "positional_inputs": [i["type"] for i in fn["inputs"] if
                                  i["name"] == ""]
        })
...
def add_eth_call_arguments(parser):
    ...
    p.add_argument("--wallet-index",
                   type=int,
                   help="Wallet index of account to use for calling "
                        "(defaults to session.identity.default_wallet_index)")
...
def add_transaction_arguments(parser):
    ...
    transaction_g.add_argument("--gas-price",
                               help="Ethereum gas price in Wei or time based "
                                    "gas price strategy "
                                    "('fast' ~1min, "
                                    "'medium' ~5min or "
                                    "'slow' ~60min)  "
                                    "(defaults to session.default_gas_price)")
    transaction_g.add_argument("--wallet-index", type=int,
                               help="Wallet index of account to use "
                                    "for signing (defaults to "
                                    "session.identity.default_wallet_index)")
    transaction_g.add_argument("--yes", "-y",
                               action="store_true",
                               help="Skip interactive confirmation "
                                    "of transaction payload",
                               default=False)
...
def add_p_mpe_address_opt(p):
    p.add_argument("--multipartyescrow-at",
                   "--mpe",
                   default=None,
                   help="Address of MultiPartyEscrow contract, if not "
                        "specified we read address from \"networks\"")
...
def add_p_registry_address_opt(p):
    p.add_argument("--registry-at",
                   "--registry",
                   default=None,
                   help="Address of Registry contract, if not specified "
                        "we read address from \"networks\"")


def add_p_metadata_file_opt(p):
    p.add_argument("--metadata-file",
                   default="service_metadata.json",
                   help="Service metadata json file "
                        "(default service_metadata.json)")
...
def add_mpe_account_options(parser):
    ...
    def add_p_snt_address_opt(_p):
        _p.add_argument("--singularitynettoken-at",
                        "--snt",
                        default=None,
                        help="Address of SingularityNetToken contract, "
                             "if not specified we read "
                             "address from 'networks'")
    ...
    p = subparsers.add_parser("balance",
                              help="Print balance of AGI tokens and "
                                   "balance of MPE wallet")
    p.set_defaults(fn="print_agi_and_mpe_balances")
    p.add_argument("--account",
                   default=None,
                   help="Account to print balance for "
                        "(default is the current identity)")
    ...
    p.add_argument("amount",
                   type=stragi2cogs,
                   help="Amount of AGI tokens to be transferred to "
                        "another account inside MPE wallet",
                   metavar="AMOUNT")
    add_p_mpe_address_opt(p)
    add_transaction_arguments(p)
...
def add_p_channel_id_opt(p):
    p.add_argument("--channel-id",
                   type=int,
                   help="The Channel Id (only in case of multiply "
                        "initialized channels for the same payment group)")
...
def add_p_group_name(p):
    p.add_argument("--group-name",
                   default=None,
                   help="Name of the payment group. Parameter "
                        "should be specified only for services with "
                        "several payment groups")


def add_p_expiration(p, is_optional):
    h = "expiration time in blocks (<int>), or in blocks related to the " \
        "current_block (+<int>blocks), or in days related to the " \
        "current_block and assuming 15 sec/block (+<int>days)"
    if is_optional:
...
def add_p_open_channel_basic(p):
    ...
    p.add_argument("--signer",
                   default=None,
                   help="Signer for the channel "
                        "(by default is equal to the sender)")
    add_p_group_name(p)
    add_p_mpe_address_opt(p)
    add_transaction_arguments(p)
    p.add_argument("--open-new-anyway",
                   action="store_true",
                   help="Skip check that channel already exists and "
                        "open new channel anyway")
    add_p_from_block(p)
...
def add_p_from_block(p):
    p.add_argument("--from-block",
                   type=int,
                   default=0,
                   help="Start searching from this block "
                        "(for channel searching)")
...
def add_mpe_channel_options(parser):
    ...
    p = subparsers.add_parser("init",
                              help="Initialize channel taking "
    ...
    p = subparsers.add_parser("open-init",
                              help="Open and initialize channel using "
    ...
    def add_p_set_for_extend_add(_p):
        ...
        expiration_amount_g.add_argument("--amount",
                                         type=stragi2cogs,
                                         help="Amount of AGI tokens to "
                                              "add to the channel")
        add_p_mpe_address_opt(p)
        add_transaction_arguments(p)

    p = subparsers.add_parser("extend-add",
                              help="Set new expiration for the channel and "
                                   "add funds")
    ...
    p = subparsers.add_parser("extend-add-for-service",
                              help="Set new expiration and add funds for "
                                   "the channel for the given service")
    ...
    def add_p_only_sender_signer(_p):
        ...
        pm.add_argument("--filter-sender",
                        action='store_true',
                        help="Print only channels in which "
                             "current identity is sender")
        pm.add_argument("--filter-signer",
                        action='store_true',
                        help="Print only channels in which "
                             "current identity is signer")
        pm.add_argument("--filter-my",
                        action='store_true',
                        help="Print only channels in which "
                             "current identity is sender or signer")

    def add_p_sender(_p):
        _p.add_argument("--sender",
                        default=None,
                        help="Account to set as sender "
                             "(by default we use the current identity)")
    ...
    p = subparsers.add_parser("print-initialized-filter-service",
                              help="Print initialized channels for "
                                   "the given service (all payment group).")
    ...
    p.add_argument("--recipient",
                   default=None,
                   help="Account to set as recipient "
                        "(by default we use the current identity)")
    ...
    p = subparsers.add_parser("claim-timeout-all",
                              help="Claim timeout for "
                                   "all channels which have "
                                   "current identity as a sender.")
...
def add_mpe_client_options(parser):
    ...
    def add_p_set1_for_call(_p):
        _p.add_argument("--service",
                        default=None,
                        help="Name of protobuf service to call. "
                             "It should be specified in case "
                             "of method name conflict.")
        _p.add_argument("method",
                        help="Target service's method name to call",
                        metavar="METHOD")
        _p.add_argument("params",
                        nargs='?',
                        help="JSON-serialized parameters object or "
                             "path containing "
                             "JSON-serialized parameters object "
                             "(leave emtpy to read from stdin)",
                        metavar="PARAMS")
        ...
        _p.add_argument("--save-field",
                        default=None,
                        nargs=2,
                        help="Save specific field in the file "
                             "(two arguments 'field' and 'file_name' should "
                             "be specified)")
        _p.add_argument("--endpoint",
                        help="Service endpoint "
                             "(by default we read it from metadata)")
        add_p_group_name(_p)

    # call: testo tests method params --endpoint --group-name
    p = subparsers.add_parser("call",
                              help="Call server. We ask state of "
                                   "the channel from the server if needed. "
                                   "Channel should be already initialized.")
    ...
    p = subparsers.add_parser("call-lowlevel",
                              help="Low level function for calling the server."
                                   " Service should be already initialized.")
...
def add_mpe_service_options(parser):
    ...
    p = subparsers.add_parser("metadata-init",
                              help="Init metadata file with providing protobuf"
                                   " directory (which we publish in IPFS) "
                                   "and display_name (optionally encoding, "
                                   "service_type and "
    ...
    p.add_argument("--payment-expiration-threshold",
                   type=int,
                   default=40320,
                   help="Service expiration threshold in blocks "
                        "(default is 40320 ~ one week with 15s/block)")

    p = subparsers.add_parser("metadata-set-model",
                              help="Publish protobuf model in ipfs "
                                   "and update existed metadata file")
    ...
    p = subparsers.add_parser("metadata-set-fixed-price",
                              help="Set pricing model as fixed "
                                   "price for all methods")
    ...
    p.add_argument("--group-name",
                   default=None,
                   help="Name of the payment group to which we want to "
                        "add endpoints. Parameter should be specified "
                        "in case of several payment groups")
    add_p_metadata_file_opt(p)
    ...
    p = subparsers.add_parser("metadata-update-endpoints",
                              help="Remove all endpoints from "
                                   "the group and add new ones")
    ...
    def add_p_publish_params(_p):
        ...
        _p.add_argument("--update-mpe-address",
                        action='store_true',
                        help="Update mpe_address in metadata before "
                             "publishing them")
        add_p_mpe_address_opt(_p)
    ...
    p = subparsers.add_parser("publish-in-ipfs",
                              help="Publish metadata only in IPFS, "
                                   "without publishing in Registry")
    p.set_defaults(fn="publish_metadata_in_ipfs")
    add_p_publish_params(p)

    p = subparsers.add_parser("update-metadata",
                              help="Publish metadata in IPFS and "
                                   "update existed service")
    ...
    p = subparsers.add_parser("print-tags",
                              help="Print tags for given service "
                                   "from registry")
    ...
    p = subparsers.add_parser("get-api-metadata",
                              help="Extract service api (model) to "
                                   "the given protodir. Get model_ipfs_hash "
                                   "from metadata")
    ...
    p = subparsers.add_parser("get-api-registry",
                              help="Extract service api (model) to "
                                   "the given protodir. Get metadata "
                                   "from registry")
...
def add_mpe_treasurer_options(parser):
    ...
    p = subparsers.add_parser("claim",
                              help="Claim given channels. We also claim all "
                                   "pending 'payments in progress' in case "
    ...
    p = subparsers.add_parser("claim-all",
                              help="Claim all channels. We also claim all "
                                   "pending 'payments in progress' in case "
                                   "we 'lost' some payments.")
    ...
    p = subparsers.add_parser("claim-expired",
                              help="Claim all channels which are close to "
                                   "expiration date. We also claim all "
                                   "pending 'payments in progress' in case "
                                   "we 'lost' some payments.")
    ...
    p.add_argument("--expiration-threshold",
                   ...
                   default=34560,
                   help="Service expiration threshold in blocks "
                        "(default is 34560 ~ 6 days with 15s/block)")
    add_p_daemon_endpoint(p)
    add_transaction_arguments(p)
...
def add_sdk_options(parser):
    ...
    p = subparsers.add_parser("generate-client-library",
                              help="Generate compiled client libraries to call"
                                   " services using your language of choice")
    p.set_defaults(fn="generate_client_library")
    p.add_argument("language",
                   choices=supported_languages,
                   help="Choose target language for the generated "
                        "client library from {}".format(supported_languages),
                   metavar="LANGUAGE")
    add_p_service_in_registry(p)
    p.add_argument("protodir",
                   nargs="?",
                   help="Directory where to output the generated "
                        "client libraries",
                   metavar="PROTO_DIR")
    add_eth_call_arguments(p)
