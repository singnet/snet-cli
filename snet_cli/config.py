...
class Config(ConfigParser):
    ...
    def safe_get_session_identity_network_names(self):
        ...
        if network and network != session_network:
            raise Exception("Your session identity '%s' is bind to "
                            "network '%s', which is different from your"
                            " session network '%s', please switch "
                            "identity or network" % (session_identity,
                                                     network,
                                                     session_network))
    ...
    def set_session_network(self, network, out_f):
        ...
        if "identity" in self["session"]:
            ...
            if identity_network and identity_network != network:
                print("Your new session network '%s' is incompatible "
                      "with your current session identity '%s' "
                      "(which is bind to network '%s'), "
                      "please switch your identity" % (network,
                                                       session_identity,
                                                       identity_network),
                      file=out_f)
    ...
    def set_session_identity(self, identity, out_f):
        ...
        if network:
            print(
                'Identity "%s" is bind to network "%s"' % (identity, network),
                file=out_f)
            self._set_session_network(network, out_f)
        else:
            print('Identity "%s" is not bind to any network. '
                  'You should switch network manually if you need.' % identity,
                  file=out_f)
        print("Switch to identity: %s" % identity, file=out_f)
        self["session"]["identity"] = identity
        self._persist()

    # session is the union of
    # session.identity + session.network + default_ipfs_endpoint
    # if value is presented in both session.identity and
    # session.network we get it from session.identity
    # (can happen only for default_eth_rpc_endpoint)
    def get_session_field(self, key, exception_if_not_found=True):
        ...
        if not rez and exception_if_not_found:
            raise Exception("Cannot find %s in the session.identity "
                            "and in the session.network" % key)
        return rez
    ...
    def set_session_field(self, key, value, out_f):
        if key == "default_ipfs_endpoint":
            ...
            print("set {}={} for network={}".format(key,
                                                    value,
                                                    session_network),
                  file=out_f)
            ...
            print("set {}={} for identity={}".format(key,
                                                     value,
                                                     session_identity),
                  file=out_f)
        else:
            all_keys = get_session_network_keys() + \
                       get_session_identity_keys() + ["default_ipfs_endpoint"]
    ...
    def unset_session_field(self, key, out_f):
        if key in get_session_network_keys_removable():
            print("unset %s from network %s" %
                  (key, self["session"]["network"]),
                  file=out_f)
    ...
    def session_to_dict(self):
        ...
        show = {"session", "network.%s" % session_network,
                "identity.%s" % session_identity, "ipfs"}
    ...
    def add_identity(self, identity_name, identity, out_f):
        ...
        if identity_section in self:
            raise Exception(
                "Identity section %s already exists in config" %
                identity_section)
        if ("network" in identity and
                identity["network"] not in self.get_all_networks_names()):
        ...
        if len(self.get_all_identities_names()) == 1:
            print("You've just added your first identity %s. "
                  "We will automatically switch to it!" % identity_name)
    ...
    def get_all_identities_names(self):
        return [x[len("identity."):] for x in self.sections() if
                x.startswith("identity.")]

    def get_all_networks_names(self):
        return [x[len("network."):] for x in self.sections() if
                x.startswith("network.")]
    ...
    def create_default_config(self):
        ...
        self["network.mainnet"] = {
            "default_eth_rpc_endpoint": "https://mainnet.infura.io",
            "default_gas_price": "medium"}
        self["network.ropsten"] = {
            "default_eth_rpc_endpoint": "https://ropsten.infura.io",
            "default_gas_price": "medium"}
...
def first_identity_message_and_exit():
    print("\nPlease create your first identity by running "
          "'snet identity create'.\n\n"
          "The available identity types are:\n"
          "    - 'rpc' (yields to a required ethereum json-rpc endpoint for "
          "signing using a given wallet\n"
          "          index)\n"
          "    - 'mnemonic' (uses a required bip39 mnemonic for "
          "HDWallet/account derivation and signing\n"
          "          using a given wallet index)\n"
          "    - 'key' (uses a required hex-encoded private key for signing)\n"
          "    - 'ledger' (yields to a required ledger nano s device "
          "for signing using a given wallet\n"
          "          index)\n"
          "    - 'trezor' (yields to a required trezor device for "
          "signing using a given wallet index)\n\n")
    exit(1)
...
def get_session_network_keys():
    return ["default_gas_price", "current_registry_at",
