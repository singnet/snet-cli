class Contract:
    ...
    def call(self, function_name, *positional_inputs, **named_inputs):
        return getattr(
            ...
            function_name)(*positional_inputs, **named_inputs).call()

    def build_transaction(self, function_name, from_address,
                          gas_price, *positional_inputs, **named_inputs):
        nonce = self.w3.eth.getTransactionCount(from_address)
        chain_id = self.w3.version.network
        return getattr(
            self.contract.functions,
            function_name)(*positional_inputs,
    ...
    def process_receipt(self, receipt):
        ...
        contract_events = map(lambda e: e["name"],
                              filter(lambda e: e["type"] == "event", self.abi))
        for contract_event in contract_events:
            events.extend(getattr(
