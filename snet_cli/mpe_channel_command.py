...
# Inherits MPEServiceCommand to use _get_service_metadata_from_registry
class MPEChannelCommand(MPEServiceCommand):
    ...
    def _get_persistent_mpe_dir(self):
        ...
        return Path.home().joinpath(".snet", "mpe_client",
                                    "%s_%s" % (mpe_address, registry_address))

    def _get_service_base_dir(self, org_id, service_id):
        """ get persistent storage for the given service
        (~/.snet/mpe_client/<mpe_address>_<registry_address>/<org_id>
        /<service_id>/) """
        return self._get_persistent_mpe_dir().joinpath(org_id, service_id)

    def get_service_spec_dir(self, org_id, service_id):
        """ get persistent storage for the given service
        (~/.snet/mpe_client/<mpe_address>/<org_id>/<service_id>
        /service/) """
        return self._get_service_base_dir(org_id, service_id).joinpath(
            "service")

    def _get_channels_info_file(self, org_id, service_id):
        return os.path.join(self._get_service_base_dir(org_id, service_id),
                            "channels_info.pickle")
    ...
    def is_service_initialized(self):
        return os.path.isfile(self._get_service_info_file(
            self.args.org_id,
            self.args.service_id))

    def _check_mpe_address_metadata(self, metadata):
        """ we make sure that MultiPartyEscrow address
        from metadata is correct """
        mpe_address = self.get_mpe_address()
        if str(mpe_address).lower() != str(metadata["mpe_address"]).lower():
            raise Exception("MultiPartyEscrow contract address "
                            "from metadata %s do not correspond to "
                            "current MultiPartyEscrow address %s" %
                            (metadata["mpe_address"], mpe_address))

    def _init_or_update_service_if_needed(self, metadata,
                                          service_registration):
        # if service was already initialized and
        # metadataURI hasn't changed we do nothing
        if self.is_service_initialized():
            if self.is_metadataURI_has_changed(service_registration):
                self._printerr("# Service with org_id=%s and "
                               "service_id=%s was updated" %
                               (self.args.org_id, self.args.service_id))
                self._printerr("# ATTENTION!!! price or other paramaters "
                               "might have been changed!\n")
            else:
                # we do nothing
                return
        self._printerr("# Initilize service with "
                       "org_id=%s and service_id=%s" %
                       (self.args.org_id, self.args.service_id))
        ...
        # it is relatevely safe to remove service_dir because we
        # know that service_dir = self.get_service_spec_dir()
        # so it is not a normal dir
    ...
    def _init_or_update_registered_service_if_needed(self):
        """
        similar to _init_or_update_service_if_needed but we
        get service_registraion from registry,
        so we can update only registered services
        """
        if self.is_service_initialized():
            old_reg = self._read_service_info(self.args.org_id,
                                              self.args.service_id)

            # metadataURI will be in old_reg only for service which
            # was initilized from registry (not from metadata)
        ...
        self._init_or_update_service_if_needed(service_metadata,
                                               service_registration)
    ...
    def _check_channel_is_mine(self, channel):
        if (channel["sender"].lower() != self.ident.address.lower() and
            ...
            raise Exception(
                "Channel does not correspond to the current "
                "Ethereum identity (address=%s sender=%s signer=%s)" %
                (self.ident.address.lower(),
    ...
    def _init_channel_from_metadata(self, metadata, service_registration):
        ...
        if group is None:
            group_id_base64 = base64.b64encode(channel["groupId"]).decode(
                'ascii')
            raise Exception("Channel %i does not correspond to "
                            "the given metadata.\n We can't find the "
                            "following group_id in metadata: %s" %
                            (channel_id, group_id_base64))
        ...
        self._add_channel_to_initialized(self.args.org_id,
                                         self.args.service_id, channel)
    ...
    def _get_expiration_from_args(self):
        ...
        We allow the following types of expiration
         ...
         2. "+<int>blocks", where <int> is integer sets expiration as:
            current_block + <int>
         3. "+<int>days", where <int> is integer sets expiration as:
            current_block + <int>*4*60*24 (we assume 15 sec/block here)

        If expiration > current_block + 1036800 (~6 month)
        we generate an exception if "--force" flag haven't been set
        """
        ...
        if rez > current_block + 1036800 and not self.args.force:
            ...
            raise Exception("You try to set expiration time too far in "
                            "the future: approximately %i days. "
                            "Set --force parameter if "
                            "you really want to do it." % d)
        return rez
    ...
    def _open_channel_for_service(self, metadata):
        ...
        if len(rez[1]) != 1 or rez[1][0]["event"] != "ChannelOpen":
            raise Exception("We've expected only one ChannelOpen event "
                            "after openChannel. Make sure that you use "
                            "correct MultiPartyEscrow address")
    ...
    def _initialize_already_opened_channel(self, metadata, sender, signer):
        ...
        for i in sorted(channels_ids):
            ...
            if channel["signer"].lower() == signer.lower():
                self._printerr("# Channel with given sender, signer and "
                               "group_id is already exists "
                               "we simply initialize it (channel_id = %i)" %
                               channel["channelId"])
                self._add_channel_to_initialized(self.args.org_id,
                                                 self.args.service_id, channel)
                return channel
    ...
    def _open_init_channel_from_metadata(self, metadata, service_registration):
        ...
        if not self.args.open_new_anyway:
            ...
            signer = self.get_address_from_arg_or_ident(self.args.signer)
            channel = self._initialize_already_opened_channel(metadata, sender,
                                                              signer)
    ...
    def channel_claim_timeout_all(self):
        ...
        for channel_id in channels_ids:
            ...
            if (rez["value"] > 0 and
                    rez["expiration"] < self.ident.w3.eth.blockNumber):
                self.transact_contract_command("MultiPartyEscrow",
                                               "channelClaimTimeout",
                                               [channel_id])
    ...
    def _smart_get_initialized_channel_for_service(self, metadata, filter_by,
                                                   is_try_initailize=True):
        """
        ...
        channels = [c for c in channels if
                    c[filter_by].lower() == self.ident.address.lower()
                    and c["groupId"] == group_id]

        if len(channels) == 0 and is_try_initailize:
            # this will work only in simple case where signer == sender
            self._initialize_already_opened_channel(metadata,
                                                    self.ident.address,
                                                    self.ident.address)
            return self._smart_get_initialized_channel_for_service(
                metadata,
                filter_by,
                is_try_initailize=False)

        if len(channels) == 0:
            raise Exception("Cannot find initialized channel for service "
                            "with org_id=%s service_id=%s and signer=%s" %
                            (self.args.org_id,
                             self.args.service_id,
                             self.ident.address))
        if self.args.channel_id is None:
            if len(channels) > 1:
                ...
                raise Exception("We have several initialized channel: %s. "
                                "You should use --channel-id to select one" %
                                str(channel_ids))
            return channels[0]
        for channel in channels:
            if channel["channelId"] == self.args.channel_id:
                return channel
        raise Exception("Channel %i has not been initialized or your are not "
                        "the sender/signer of it" % self.args.channel_id)

    def _smart_get_channel_for_service(self):
        self._init_or_update_registered_service_if_needed()
        metadata = self._read_metadata_for_service(self.args.org_id,
                                                   self.args.service_id)
        return self._smart_get_initialized_channel_for_service(metadata,
                                                               "sender")
    ...
    def _get_all_initialized_channels(self):
        ...
        rez[(<org_id>, <service_id>)] = [(channel_id, channel_info)] """
        ...
        for service_base_dir in self._get_persistent_mpe_dir().glob("*/*"):
            ...
            channels = self._get_initialized_channels_for_service(org_id,
                                                                  service_id)
            if channels:
    ...
    def _read_metadata_for_service(self, org_id, service_id):
        ...
        if not os.path.exists(sdir):
            raise Exception("Service with org_id=%s and service_id=%s "
                            "is not initialized" % (org_id, service_id))
    ...
    def _print_channels_from_blockchain(self, channels_ids):
        ...
        if self.args.only_id:
            ...
            return
        self._printout("#channelId nonce recipient groupId(base64) "
                       "value(AGI) expiration(blocks)")
    ...
    def _print_channels_dict_from_blockchain(self, channels_dict):
        ...
        if self.args.only_id:
            ...
            self._printout("#organization_id service_id group_name "
                           "channel_id nonce value(AGI) expiration(blocks)")
        for org_id, service_id in channels_dict:
            ...
            for channel in channels:
                ...
                if self.args.only_id:
                    ...
                    channel_blockchain = \
                        self._get_channel_state_from_blockchain(channel_id)
    ...
    def _get_all_filtered_channels(self, topics_without_signature):
        """ get all filtered channels from blockchain logs """
        mpe_address = self.get_mpe_address()
        event_signature = self.ident.w3.sha3(
            text="ChannelOpen(uint256,uint256,address,address,"
                 "address,bytes32,uint256,uint256)").hex()
        ...
        channels_ids = [get_event_data(event_abi, l)["args"]["channelId"]
                        for l in logs]
        return channels_ids
    ...
    def _get_all_channels_filter_sender_recipient_group(self, sender,
                                                        recipient, group_id):
        sender_padded = pad_hex(sender.lower(), 256)
