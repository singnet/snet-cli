...
# Inherits MPEChannelCommand because client needs channels
class MPEClientCommand(MPEChannelCommand):
    ...
    def _get_call_params(self):
        ...
        if Path(params_string).is_file():
            self._printerr(
                "Read call params from the file: %s" % params_string)
        ...
        try:
            ...
            self._printerr("Decoding JSON has failed "
            ...
            if isinstance(v, dict):
                ...
                for m in k_mods:
                    if m == "file":
                        ...
                        raise Exception("Unknown modifier ('%s') in "
                                        "call parameters. "
                                        "Possible modifiers: "
                                        "file, b64encode, b64decode" % m)
            rez[k_final] = v
        return rez
    ...
    def _call_server_via_grpc_channel(self, grpc_channel, channel_id, nonce,
        ...
        stub_class, request_class, response_class = \
            self._import_protobuf_for_service()
    ...
    def _get_endpoint_from_metadata_or_args(self, metadata):
        ...
        if not endpoints:
            raise Exception("Cannot find endpoint in metadata "
                            "for the given payment group.")
        if len(endpoints) > 1:
            self._printerr("There are several endpoints "
                           "for the given payment group. We will select %s" %
                           endpoints[0])
        return endpoints[0]

    def call_server_lowlevel(self):
        params = self._get_call_params()
        service_metadata = self._read_metadata_for_service(
            self.args.org_id,
            self.args.service_id)
    ...
    def _get_channel_state_from_server(self, grpc_channel, channel_id):
        ...
        codegen_dir = Path.home().joinpath(".snet",
                                           "mpe_client",
                                           "state_service")
        ...
        request = request_class(channel_id=self.w3.toBytes(channel_id),
        ...
        if state["current_signed_amount"] > 0:
            good = self._verify_my_signature(bytes(response.current_signature),
                                             self.get_mpe_address(),
                                             channel_id,
                                             state["current_nonce"],
                                             state["current_signed_amount"])
            if not good:
                raise Exception("Error in _get_channel_state_from_server. "
                                "My own signature from the server "
                                "is not valid.")
    ...
    def _get_channel_state_statelessly(self, grpc_channel, channel_id):
        ...
        https://github.com/singnet/wiki/blob/master/multiPartyEscrowContract
        /MultiPartyEscrow_stateless_client.md
        """
        ...
        if server["current_nonce"] == blockchain["nonce"]:
            unspent_amount = blockchain["value"] - server[
                "current_signed_amount"]
        else:
            # in this case we cannot securely define unspent_amount yet
            unspent_amount = None

        return (server["current_nonce"], server["current_signed_amount"],
                unspent_amount)

    def print_channel_state_statelessly(self):
        grpc_channel = open_grpc_channel(self.args.endpoint)

        current_nonce, current_amount, unspent_amount = \
            self._get_channel_state_statelessly(grpc_channel,
                                                self.args.channel_id)
        self._printout("current_nonce                  = %i" % current_nonce)
        self._printout("current_signed_amount_in_cogs  = %i" % current_amount)
        self._printout(
            "current_unspent_amount_in_cogs = %s" % str(unspent_amount))

    @staticmethod
    def _get_price_from_metadata(service_metadata):
        pricing = service_metadata["pricing"]
        if pricing["price_model"] == "fixed_price":
            return pricing["price_in_cogs"]
        raise Exception(
            "We do not support price model: %s" % (pricing["price_model"]))

    def call_server_statelessly_with_params(self, params):
        # if service is not initilized we will initialize it
        # (unless we want skip registry check for update)
        ...
        # if channel was not initilized we will try to initailize it
        # (it will work only in simple case of signer == sender)
        channel = self._smart_get_initialized_channel_for_service(
            service_metadata, filter_by="signer")
        channel_id = channel["channelId"]
        price = self._get_price_from_metadata(service_metadata)
        server_state = self._get_channel_state_from_server(grpc_channel,
                                                           channel_id)

        proceed = self.args.yes or input("Price for this call will be %s AGI "
                                         "(use -y to remove this warning). "
                                         "Proceed? (y/n): " %
                                         (cogs2stragi(price))) == "y"
        if not proceed:
