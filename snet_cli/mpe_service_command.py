...
from snet_cli.utils_ipfs import (hash_to_bytesuri,
                                 bytesuri_to_hash,
...
class MPEServiceCommand(BlockchainCommand):
    ...
    def _metadata_add_group(self, metadata):
        if not web3.eth.is_checksum_address(self.args.payment_address):
            raise Exception("payment_address parameter is not a valid "
    ...
    def metadata_add_description(self):
        ...
        if self.args.url:
            if "url" in service_description:
                raise Exception("json service description "
                                "already contains url field")
            service_description["url"] = self.args.url
        if self.args.description:
            if "description" in service_description:
                raise Exception("json service description "
                                "already contains description field")
    ...
    def _publish_metadata_in_ipfs(self, metadata_file):
        ...
        if mpe_address.lower() != metadata["mpe_address"].lower():
            raise Exception(
                "\n\nmpe_address in metadata does not correspond to "
                "the current MultiPartyEscrow contract address\n" +
                "You have two possibilities:\n" +
                "1. You can use --multipartyescrow-at "
                "to set current mpe address\n" +
                "2. You can use --update-mpe-address parameter "
                "to update mpe_address in metadata before publishing it\n")
        return self._get_ipfs_client().add_bytes(
            metadata.get_json().encode("utf-8"))
    ...
    def publish_metadata_in_ipfs_and_update_registration(self):
        # first we check that we do not change payment_address
        ...
        for old_group in old_metadata["groups"]:
            if new_metadata.is_group_name_exists(old_group["group_name"]):
                ...
                if (new_group["group_id"] != old_group["group_id"] or
                        new_group["payment_address"] !=
                        old_group["payment_address"]):
                    raise Exception(
                        "\n\nYou are trying to change group_id "
                        "or payment_address in group '%s'.\n" %
                        old_group["group_name"] +
                        "It would make all open "
                        "channels invalid.\n" +
                        "You shoudn't use 'metadata-init' for "
                        "existed service, because it "
                        "reinitialize group_id\n" +
                        "Please use 'metadata-set-model' "
                        "for change your protobuf file\n" +
                        "Please use "
                        "'metadata-remove-all-endpoints/metadata-add-endpoints"
                        " to update endpoints'\n\n")
        metadata_uri = hash_to_bytesuri(
            self._publish_metadata_in_ipfs(self.args.metadata_file))
        params = [type_converter("bytes32")(self.args.org_id),
    ...
    def update_registration_add_tags(self):
        ...
        self.transact_contract_command("Registry",
                                       "addTagsToServiceRegistration",
                                       params)

    def update_registration_remove_tags(self):
        params = self._get_params_for_tags_update()
        self.transact_contract_command("Registry",
                                       "removeTagsFromServiceRegistration",
                                       params)
    ...
    def _get_service_registration(self):
        ...
        rez = self.call_contract_command("Registry",
                                         ...
                                         params)
        if rez[0] == False:
            raise Exception("Cannot find Service with id=%s in Organization "
                            "with id=%s" % (self.args.service_id,
                                            self.args.org_id))
