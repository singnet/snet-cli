...
payment_expiration_threshold - Service will reject payments with expiration
                                less than
                                current_block + payment_expiration_threshold.
                               This field should be used by the client
                               with caution. Client should not
                               accept arbitrary payment_expiration_threshold
model_ipfs_hash  - IPFS HASH to the .tar archive of
                    protobuf service specification
mpe_address      - Address of MultiPartyEscrow contract.
                   Client should use it exclusively for
                   cross-checking of mpe_address,
                   (because service can attack via mpe_address)
                   Daemon can use it directly if authenticity
                   of metadata is confirmed
pricing {}      -  Pricing model
         Possible pricing models:
         1. Fixed price
             price_model   - "fixed_price"
             price_in_cogs -  unique fixed price in cogs for all method
             (1 AGI = 10^8 cogs)
             (other pricing models can be easily supported)
groups[]       - group is the number of endpoints which shares
                    same payment channel;
                  grouping strategy is defined by service provider;
                  for example service provider can use
                  region name as group name
     group_name - unique name of the group (human readable)
     group_id   - unique id of the group
                    (random 32 byte string in base64 encoding)
...
class MPEServiceMetadata:
    ...
    def set_simple_field(self, f, v):
        if (
                f != "display_name" and
                f != "encoding" and
                f != "model_ipfs_hash" and
                f != "mpe_address" and
                f != "service_type" and
                f != "payment_expiration_threshold" and
    ...
    def add_group(self, group_name, payment_address):
        ...
        if self.is_group_name_exists(group_name):
            raise Exception(
                "the group \"%s\" is already present" % str(group_name))
    ...
    def add_endpoint(self, group_name, endpoint):
        if re.match(r"^\w+://", endpoint) is None:
            # TODO: Default to https when our tutorials show
            #  setting up a ssl certificate as well
        ...
        if endpoint in self.get_all_endpoints():
            raise Exception(
                "the endpoint %s is already present" % str(endpoint))
        self.m["endpoints"] += \
            [{"group_name": group_name, "endpoint": endpoint}]

    def remove_all_endpoints(self):
        self.m["endpoints"] = []

    def remove_all_endpoints_for_group(self, group_name):
        self.m["endpoints"] = [e for e in self.m["endpoints"] if
    ...
    def set_from_json(self, j):
        # TODO: we probaly should check the  consistency of loaded json here
    ...
    def get_group_name_nonetrick(self, group_name=None):
        """
        In all getter function in case of single payment group,
        group_name can be None
        """
        groups = self.m["groups"]
        if len(groups) == 0:
        ...
        if not group_name:
            if len(groups) > 1:
                raise Exception("We have more than one payment group in "
                                "metadata, so group_name should be specified")
    ...
    def get_endpoints_for_group(self, group_name=None):
        group_name = self.get_group_name_nonetrick(group_name)
        return [e["endpoint"] for e in self.m["endpoints"] if
                e["group_name"] == group_name]
