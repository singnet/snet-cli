...
class MPETreasurerCommand(MPEClientCommand):
    """
    We inherit MPEChannelCommand because we need
    _get_channel_state_from_blockchain
    """
    ...
    def _sign_message_start_claim(self, mpe_address, channel_id,
                                  channel_nonce):
        message = self.w3.soliditySha3(
    ...
    def _get_stub_and_request_classes(self, service_name):
        ...
        stub_class, request_class, _ = import_protobuf_from_dir(codegen_dir,
                                                                service_name)
    ...
    def _call_GetListUnclaimed(self, grpc_channel):
        ...
        signature = self._sign_message_list_unclaimed(mpe_address,
                                                      current_block)
        request = request_class(mpe_address=mpe_address,
        ...
        for p in response.payments:
            if len(p.signature) > 0:
                raise Exception("Signature was set in "
                                "GetListUnclaimed. Response is invalid")
    ...
    def _call_GetListInProgress(self, grpc_channel):
        ...
        mpe_address = self.get_mpe_address()
        current_block = self.ident.w3.eth.blockNumber
        signature = self._sign_message_list_in_progress(mpe_address,
                                                        current_block)
        request = request_class(mpe_address=mpe_address,
                                current_block=current_block,
                                signature=bytes(signature))
    ...
    def _call_StartClaim(self, grpc_channel, channel_id, channel_nonce):
        ...
        stub = stub_class(grpc_channel)
        mpe_address = self.get_mpe_address()
        signature = self._sign_message_start_claim(mpe_address, channel_id,
                                                   channel_nonce)
        request = request_class(mpe_address=mpe_address,
    ...
    def _blockchain_claim(self, payments):
        for payment in payments:
            ...
            if len(sig) != 65:
                raise Exception("Length of signature is incorrect: "
                                "%i instead of 65" % (len(sig)))
    ...
    def _start_claim_channels(self, grpc_channel, channels_ids):
        ...
        unclaimed_payments_dict = {p["channel_id"]: p for p in
                                   unclaimed_payments}

        to_claim = []
        for channel_id in channels_ids:
            if (channel_id not in unclaimed_payments_dict or
                    unclaimed_payments_dict[channel_id]["amount"] == 0):
                self._printout("There is nothing to claim for channel %i, "
                               "we skip it" % channel_id)
                continue
            blockchain = self._get_channel_state_from_blockchain(channel_id)
            if (unclaimed_payments_dict[channel_id]["nonce"] !=
                    blockchain["nonce"]):
                self._printout("Old payment for channel %i is still "
                               "in progress. Please run claim for "
                               "this channel later." % channel_id)
                continue
            to_claim.append((channel_id, blockchain["nonce"]))

        payments = [self._call_StartClaim(grpc_channel, channel_id, nonce) for
                    channel_id, nonce in to_claim]
        return payments

    def _claim_in_progress_and_claim_channels(self, grpc_channel, channels):
        """
        Claim all 'pending' payments in progress
        and after we claim given channels
        """
        # first we get the list of all 'payments in progress'
        # in case we 'lost' some payments.
        payments = self._call_GetListInProgress(grpc_channel)
        if len(payments) > 0:
            self._printout("There are %i payments in 'progress' "
                           "(they haven't been claimed in blockchain). "
                           "We will claim them." % len(payments))
    ...
    def claim_almost_expired_channels(self):
        ...
        for p in unclaimed_payments:
            ...
            blockchain = self._get_channel_state_from_blockchain(channel_id)
            if (blockchain["expiration"] <
                    self.ident.w3.eth.blockNumber +
                    self.args.expiration_threshold):
