...
from snet_cli.utils_ipfs import (bytesuri_to_hash,
...
class SDKCommand(MPEServiceCommand):
    def generate_client_library(self):
        ...
        if not self.args.protodir:
            client_libraries_base_dir_path = cur_dir_path.joinpath(
                "client_libraries")
            ...
            if not os.path.isdir(client_libraries_base_dir_path):
                self._error(
                    ...
                    "specified path exists".format(
                        client_libraries_base_dir_path))
        ...
        library_dir_path = client_libraries_base_dir_path.joinpath(
            library_language, get_contract_address(self, "Registry"),
            library_org_id, library_service_id)
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            proto_temp_dir_path = temp_dir_path.joinpath(library_language,
                                                         library_org_id,
                                                         library_service_id)
            safe_extract_proto_from_ipfs(self._get_ipfs_client(),
                                         model_ipfs_hash, proto_temp_dir_path)

            # Compile proto files
            compile_proto(Path(proto_temp_dir_path), library_dir_path)

        self._printout('client libraries for service '
                       'with id "{}" in org with id "{}" '
                       'generated at {}'.format(library_service_id,
                                                library_org_id,
                                                library_dir_path))
