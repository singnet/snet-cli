...
def get_contract_address(cmd, contract_name, error_message=None):
    ...
    _error = "Fail to read %s address from 'networks', " \
             "you should specify address by yourself via " \
             "--%s_at parameter" % (contract_name, contract_name.lower())
    error_message = error_message or _error
    # try to take address from networks
    return read_default_contract_address(w3=cmd.w3,
...
def get_field_from_args_or_session(config, args, field_name):
    ...
    # type(rez) can be int in case of wallet-index,
    # so we cannot make simply if(rez)
    if rez is not None:
        return rez
    rez = config.get_session_field("default_%s" % field_name,
                                   exception_if_not_found=False)
    if rez:
        return rez
