...
def publish_proto_in_ipfs(ipfs_client, protodir):
    ...
    # We are sorting files before we add them to
    # the .tar since an archive containing the same files in a different
...
def get_from_ipfs_and_checkhash(ipfs_client, ipfs_hash_base58, validate=True):
    """
    Get file from ipfs
    We must check the hash becasue we cannot believe that
    ipfs_client wasn't been compromise
    """
    if validate:
        ...
        assert (unixfs_data.Type ==
                unixfs_data.DataType.Value('File'), "IPFS hash must be a file")
...
def hash_to_bytesuri(s):
    ...
    """
    # TODO: we should pad string with zeros till closest
    #  32 bytes word because of a bug in processReceipt
    #  (in snet_cli.contract.process_receipt)
    s = "ipfs://" + s
...
def safe_extract_proto_from_ipfs(ipfs_client, ipfs_hash, protodir):
    """
    Tar files might be dangerous
    (see https://bugs.python.org/issue21109,
    and
    https://docs.python.org/3/library/tarfile.html, TarFile.extractall warning)
