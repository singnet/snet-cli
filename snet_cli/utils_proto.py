...
def import_protobuf_from_dir(proto_dir, method_name, service_name=None):
    ...
    service_name should be provided only in the case of conflicting
    method names (two methods with the same name in difference services).
    ...
    # <SERVICE>_pb2_grpc.py import <SERVICE>_pb2.py
    # so we are forced to add proto_dir to path
    sys.path.append(str(proto_dir))
    grpc_pyfiles = [str(os.path.basename(p)) for p in
                    proto_dir.glob("*_pb2_grpc.py")]

    good_rez = []
    for grpc_pyfile in grpc_pyfiles:
        is_found, rez = _import_protobuf_from_file(grpc_pyfile, method_name,
                                                   service_name)
        if is_found:
            good_rez.append(rez)
    if len(good_rez) == 0:
        raise Exception("Error while loading protobuf. "
                        "Cannot find method=%s" % method_name)
    if len(good_rez) > 1:
        if service_name:
            raise Exception("Error while loading protobuf. "
                            "Found method %s.%s in multiply .proto files. "
                            "We don't support packages yet!" %
                            (service_name, method_name))
        else:
            raise Exception("Error while loading protobuf. "
                            "Found method %s in multiply .proto files. "
                            "You could try to specify service_name." %
                            method_name)
    return good_rez[0]
...
def _import_protobuf_from_file(grpc_pyfile, method_name, service_name=None):
    ...
    return (False, None) if fails
    return (True, (stub_class, request_class, response_class)) if success
    """
    ...
    # we take all objects from pb2_grpc module which endswith "Stub",
    # and we remove this postfix to get service_name
    all_service_names = [stub_name[:-4] for stub_name in dir(pb2_grpc) if
                         stub_name.endswith("Stub")]
    ...
    for service_name in all_service_names:
        service_descriptor = getattr(pb2, "DESCRIPTOR").services_by_name[
            service_name]
    ...
    if len(found_services) > 1:
        raise Exception("Error while loading protobuf. "
                        "We found methods %s in multiply services [%s]."
                        " You should specify service_name." %
                        (method_name, ", ".join(found_services)))
...
def switch_to_json_payload_encoding(call_fn, response_class):
    ...
    def json_serializer(*args):
        return bytes(json_format.MessageToJson(
            args[0],
            True,
            preserving_proto_field_name=True),
            "utf-8")

    def json_deserializer(*args):
        resp = response_class()
